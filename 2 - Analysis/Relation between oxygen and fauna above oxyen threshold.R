...
# for all sampling stations above 3 ml O2 per liter (supplement)
...
# create the plot
pdf(paste(getwd(),"3 - Outputs/subset_Oxygen_biomass_abundance.pdf",sep="/"),width=7,height=6) 
...
plot(Env$BC_bio~Env$oxygen,las=1, ylab=TeX("Biomass (g ww per m^{2})"),xlab="",
     pch=16,cex=1.3,main="Box core",xlim=c(3,6),xaxt="n",ylim=c(0,130),yaxt="n")
axis(1,c(3,4.5,6))
axis(2,c(0,30,60,90,120),las=1)
text("(a)",x=3.2,y=120)

m1 <- lm(BC_bio ~ oxygen, data = Env)
oxygen <- seq(3,6,0.01)
...
plot(log10(Env$Dr_bio+1)~Env$oxygen,las=1, ylab="Biomass (g ww per trawl)",xlab="",
     pch=16,cex=1.3,yaxt="n",main = "Dredge",xlim=c(3,6),xaxt="n",ylim=c(0,4))
axis(1,c(3,4.5,6))
axis(2,c(log10(0+1),log10(10+1),log10(100+1),log10(1000+1),log10(10000+1)),
     c("0","10","100","1000","10000"),las=1)
text("(b)",x=3.2,y=log10(6000+1))
Env$Logdrbio <- log10(Env$Dr_bio+1)

m1 <- lm(Logdrbio~oxygen, data = Env)
oxygen <- seq(3,6,0.01)
newdat <- data.frame(oxygen)
newdat[,2] <- predict(m1,newdata=newdat)
lines(newdat[,2]~newdat[,1],col="black",lty=5)
...
# abundance box core
plot(Env$BC_abu~Env$oxygen,las=1, ylab=TeX("Individuals (per m^{2})"),xlab=""
     ,pch=16,cex=1.3,ylim=c(0,1300),xlim=c(3,6),xaxt="n",yaxt="n")
axis(1,c(3,4.5,6))
axis(2,c(0,300,600,900,1200),las=1)
text("(c)",x=3.2,y=1200)

m1 <- lm(BC_abu~oxygen, data = Env)
oxygen <- seq(3,6,0.01)
newdat <- data.frame(oxygen)
newdat[,2] <- predict(m1,newdata=newdat)
lines(newdat[,2]~newdat[,1],col="black",lty=5)
...
# abundance dredge
plot(log10(Env$Dr_abu+1)~Env$oxygen,las=1, ylab="Individuals (per trawl)",xlab="",
     pch=16,cex=1.3,yaxt="n",xlim=c(3,6),xaxt="n")
axis(1,c(3,4.5,6))
axis(2,c(log10(0+1),log10(10+1),log10(100+1),log10(1000+1),log10(10000+1)),
     c("0","10","100","1000","10000"),las=1)
text("(d)",x=3.2,y=log10(8000+1))

m1 <- lm(log10(Dr_abu+1)~oxygen, data = Env)
oxygen <- seq(3,6,0.01)
newdat <- data.frame(oxygen)
newdat[,2] <- predict(m1,newdata=newdat)
lines(newdat[,2]~newdat[,1],col="black",lty=5)
