...
# abundance box core
  plot(Env$BC_abu~Env$SAR_1317,las=1, ylab=TeX("Individuals (per 0.3 m^{2})"),xlab="Fishing intensity (y-1)",
       pch=16,cex=1.3,ylim=c(0,400),col=pal(nrow(Env))[Env$order])
...
# abundance dredge
  plot(log10(Env$Dr_abu+1)~Env$SAR_1317,las=1, ylab="Individuals (per trawl)",xlab="Fishing intensity (y-1)",
       pch=16,cex=1.3,yaxt="n",col=pal(nrow(Env))[Env$order])
  axis(2,c(log10(0+1),log10(10+1),log10(100+1),log10(1000+1),log10(10000+1)),
       c("0","10","100","1000","10000"),las=1)
